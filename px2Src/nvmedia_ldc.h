...
typedef enum {
    ...
    /*! Specifies to do temporal noise reduction. */
    NVMEDIA_LDC_MODE_TNR3,

    /*! Specifies to do both geometric transform and temporal noise reduction. */
 ...
 *    \ref NVMEDIA_LDC_MODE_GEOTRANS or \ref NVMEDIA_LDC_MODE_GEOTRANS_TNR3.
 *    It uses NvMediaLDCInitParams::geoTransParams to fetch the pixel defined by srcRect
 *    in the source image and renders onto the destination
 *    rectangle of the destination image.
 *    The source image and the destination image must have
 *    the same format. LDC bypasses the transform stage if geometric transform is disabled.
 *    The region of interest in the destination image is defined by:
 *    - Destination rectangle and
 *    - NvMediaLDCBitMaskMap surface (if \ref NvMediaGeoTransParams::bitMaskEnable is set).
 *
 * 2. LDC outputs \a xSobel if:
 *    - NvMediaLDCInitParams::ldcMode is \ref NVMEDIA_LDC_MODE_GEOTRANS and
 *    - \ref NvMediaLDCCtrlParams::xSobelMode is \ref NVMEDIA_GEOTRANS_ENABLE_XSOBEL_ENABLE_DS or
 *      \ref NVMEDIA_GEOTRANS_ENABLE_XSOBEL_DISABLE_DS
 *
 *    The xSobel image must have the same bit-depth as the source image.
 *    NvMediaLDCCtrlParams::xSobelMode must be NULL if NvMediaLDCInitParams::ldcMode is not
 ...
 *
 * If NvMediaLDCInitParams::ldcMode is \ref NVMEDIA_LDC_MODE_TNR3 or \ref NVMEDIA_LDC_MODE_GEOTRANS_TNR3,
 * LDC performs temporal noise reduction.
 *
 * NvMediaLDCProcess() is a non-blocking call.
 *
 * NvMediaImageGetStatus() can be used to check the status of input/output images.
 *
 * \param[in] ldc NvMediaLDC Handle obtained by call to \ref NvMediaLDCCreate().
 * \param[in] prevSurface A pointer to the previous image output.
 *            Used only when NvMediaLDCInitParams::ldcMode is \ref NVMEDIA_LDC_MODE_GEOTRANS_TNR3 or
 *            \ref NVMEDIA_LDC_MODE_TNR3
 * \param[in] curSurface A pointer to the source image.
 * \param[in,out] outputSurface A pointer to the output image. Caller must create this surface.
 * \param[inout] xSobel A pointer to the xSobel image output. Caller must create this surface.
 * \param[in,out] downSample A pointer to the downsampled image output. Caller must create this surface.
 * \param[in] ldcCtrlParams A pointer to run-time control parameters.
 * \return \ref NvMediaStatus.
 * Possible values are:
 * \n \ref NVMEDIA_STATUS_OK - Success.
 * \n \ref NVMEDIA_STATUS_NOT_INITIALIZED - Invalid state.
 * \n \ref NVMEDIA_STATUS_BAD_PARAMETER - Invalid input argument(s).
 * \n \ref NVMEDIA_STATUS_NOT_SUPPORTED - Not supported in current config.
 * \n \ref NVMEDIA_STATUS_OUT_OF_MEMORY - Insufficient memory
 * \n \ref NVMEDIA_STATUS_ERROR - A catch-all error, used when no other error code applies.
 */
NvMediaStatus
NvMediaLDCProcess(
    NvMediaLDC *ldc,
